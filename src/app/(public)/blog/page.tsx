        ...
        <div className="container-main">
          {posts.length === 0 ? (
            <div className="text-center py-16">
              <p className="text-brand-muted text-lg mb-4">No articles published yet.</p>
              <p className="text-brand-muted text-sm">Check back soon for news and updates!</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {posts.map((post) => (
                <article key={post.id} className="bg-brand-surface rounded-xl border border-[rgba(255,255,255,0.05)] overflow-hidden hover:border-brand-cyan/30 transition-all duration-300 hover:-translate-y-1 group">
                  {post.featuredImage && (
                  ...
                  <div className="p-6">
                    ...
                    <h3 className="text-lg font-bold text-brand-white mt-2 mb-3 line-clamp-2 group-hover:text-brand-cyan transition-colors">
                      <Link href={`/blog/${post.slug}`}>{post.title}</Link>
                    </h3>
                    {post.excerpt && (
                      <p className="text-sm text-brand-muted line-clamp-3 mb-4">{post.excerpt}</p>
                    )}
