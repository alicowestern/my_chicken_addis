        ...
        <div className="container-main">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <Card key={feed.title} hover className="flex flex-col bg-white border-brand-gray-200 p-8">
                <div className="w-14 h-14 rounded-full bg-brand-cyan/10 text-brand-cyan flex items-center justify-center mb-6">
                  <Wheat className="w-7 h-7" />
                </div>
                <h3 className="text-2xl font-bold text-brand-gray-900 mb-2">{feed.title}</h3>
                <p className="text-brand-cyan font-bold tracking-widest text-sm uppercase mb-4">{feed.stage}</p>
                <p className="text-brand-gray-600 leading-relaxed mb-8 flex-grow">{feed.description}</p>
                
                <ul className="space-y-3 mb-8">
                  {feed.features.map((feature, i) => (
                    <li key={i} className="flex items-center text-sm text-brand-gray-500">
